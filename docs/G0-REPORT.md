...
G0 的主要接口可行性已获得证据。2026-09-08 补充了固定供应商响应经过真实宿主计量、HTTP 通道和 Chrome 界面绘制的计时：三次 usage 发出到包含对应用量行的截图完成，上界分别为 137、76、86 ms，均小于 1 秒。该项不再仅以测试标记的 HTTP 延迟代替界面证据；这是本机受控响应的显示链路验证，不是 DeepSeek 在线流的时延保证。首批在线请求的成功范围与限制仍见下文。
...
| 快照通知与绘制 | 标记 HTTP 更新 4–5 ms；本地固定供应商 usage 经生产计量和真实浏览器到截图完成为 137、76、86 ms | 截图上界包含 RPC 返回、DOM 校验与合成；没有测量 DeepSeek 在线流的同一时延 |
...
- `scripts/latency-probe.ts` 配合 `scripts/browser-latency.mjs`：在独立 DSH_HOME 中注册仅本地的固定 Adapter，经生产计量与 RPC 显示 130 Token；真实 Chrome 校验对应请求行并截图，断言 usage 到截图完成小于 1 秒。使用 `node scripts/build.mjs --latency` 构建测试辅助模块，不随安装包发布。浏览器脚本复用固定 Harness 的 Playwright 和本机 Chrome；启动日志路径可作为首个参数。
...
显示补测使用独立 `.cache/latency-home`，没有改写首批账本或重置其预算。三次固定响应的 390 Token 是夹具数据，没有外部模型请求；原始时间及截图保存于 `.cache/latency-evidence/`。核对首张截图中的请求行确实显示 130 Token 后，已停止本次 PID 69619，确认测试端口 63372 无监听；浏览器由测试脚本的 finally 关闭。本次只停止自己创建的服务。

用户已授权开始 G1，并先补齐必要的 G0 验收。下一步细化 G1 的任务信号、受管 Skill 清单、分析与授权 schema，并接入已确认的主页面右侧面板。本批 20 分钟在线窗口已结束；如需要额外真实调用，须先确定新的具体批次和预算，不能清空旧账本续用。
