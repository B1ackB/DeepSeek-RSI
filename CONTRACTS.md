...
更新日期：2026-09-08。状态：用户已选择分阶段确定契约，并确认 §8 的测试 Skill、首批调用预算及后续安装 OrbStack；本文是实现前草案，没有可执行 schema 或 G0 通过证据。
...
4. 有效版本与搜索候选分离。用户另行批准精确候选后才能启用，旧会话继续使用已绑定版本；覆盖原目录需要独立同意。
...
本节记录已核对的环境及用户确认的执行方案。用户已指定后续安装 OrbStack，并同意 G0 首批模型调用预算；本轮仅完成规划，没有安装、启动或付费执行。进入 G0 实施后沿用这些授权，不重复确认相同事项；超出预算或改变范围需重新确认。
