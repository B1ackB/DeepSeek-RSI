...
export function openStore(file: string) {
	...
	if (schema !== 0 && schema !== 1 && schema !== 2 && schema !== 3 && schema !== 4 && schema !== 5) { db.close(); throw fault('schema_version', 'Unsupported RSI database version'); }
	try { db.exec(`BEGIN IMMEDIATE;
		...
		if (previous) {
			...
			if (schema !== 5) db.prepare('UPDATE g1_state SET data=? WHERE id=1').run(JSON.stringify(value));
		}
		if (schema !== 5) {
	...
	function reserve(attempt: Attempt) {
		...
		transaction(() => {
			...
			if (attempt.owner === 'rsi' && attempt.purpose !== 'probe') {
				...
				const task = flow ?? analysis;
				if (!task || task.deadline === null || (flow ? flow.status !== 'generating' || attempt.purpose !== 'generation' : analysis?.status !== 'running' || attempt.purpose !== 'analysis')) throw fault('not_authorized', '任务尚未确认或已停止');
				...
				if (rows.length >= task.budget.maxRequests || (flow && rows.reduce((sum, a) => sum + (a.tokens.total ?? 0), 0) >= flow.budget.tokenStopThreshold)) throw fault('budget_exhausted', '任务调用或 Token 停止阈值已到');
