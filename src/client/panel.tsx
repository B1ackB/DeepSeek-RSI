...
export function RsiPanel({ useRsi, useSessions, useWorkspaces, command, catalog, openSession }: PanelProps) {
	...
	return <aside className="rsi-panel" aria-label="RSI 控制面板">
		...
		`}</style>
		<div><h2>Skill 优化</h2><p className="rsi-muted">提交前端需求后，在这里确认偏好和预算；审阅 Skill 候选后再开始页面。</p></div>
		...
		{snapshot ? <>
			...
			<section className="rsi-card" aria-label="本次迭代范围"><h3>本次任务</h3>
				{enrollment ? <><strong>{enrollment.decision === 'included' ? `已纳入：${snapshot.business.skills.find(s => s.id === enrollment.skillId)?.name ?? 'Skill'}` : '本次暂不纳入 RSI'}</strong><p className="rsi-muted">连续修改沿用本次选择；实际修改 Skill 和启用候选仍需另行授权。</p><button disabled={disabled} onClick={() => void run('end_enrollment', { enrollmentId: enrollment.id })}>结束本次任务</button><small>只结束 RSI 观察范围，普通编写任务继续。</small></> : <p>在聊天框描述要编写的前端页面，或使用 /Skill名称。无需预先登记；修改确认卡会出现在此面板。</p>}
			</section>
			...
			<section><h3>受管 Skill</h3>{skills.length === 0 && <p className="rsi-muted">尚未纳入 Skill。选择下方已有技能后才开始观察。</p>}
				{skills.map(skill => <div className="rsi-card" key={skill.id}>
					<strong>{skill.name}</strong><small>{skill.snapshot.versionDigest.slice(0, 12)} · {skill.snapshot.files.length} 个文件 · {skill.sourceState === 'matching' ? '来源已核对' : '来源需要核对'}</small>
			...
			<section><h3>优化机会</h3>{snapshot.business.observationError && <p role="alert">{snapshot.business.observationError}</p>}
				...
				{snapshot.business.opportunities.filter(o => skillIds.has(o.skillId)).map(opportunity => <details className="rsi-card" key={opportunity.id} open={opportunity.status === 'open'}>
					...
					{opportunity.evidence.map(e => <p key={`${e.sessionId}:${e.seq}`}><span>{e.summary}</span><small>事件 {e.seq} · {new Date(e.time).toLocaleString()}</small></p>)}
			...
			{analyses.length > 0 && <section><h3>分析与方向</h3><select aria-label="选定 RSI 分析" value={analysis?.id ?? ''} onChange={e => setAnalysis(e.target.value)}>{analyses.map(a => <option key={a.id} value={a.id}>{new Date(a.createdAt).toLocaleTimeString()} · {statuses[a.status]}</option>)}</select>
				{analysis && <><strong>{statuses[analysis.status]}</strong>{analysis.error && <p role="alert">{analysis.error}</p>}
					<details open={analysis.status === 'prepared'}><summary>本次发送范围与预算</summary><p>DeepSeek-V4-Flash / {efforts[analysis.budget.reasoningEffort]} · 最多 {analysis.budget.maxRequests} 次请求，输出（含推理）{analysis.budget.maxOutputTokens} Token，{analysis.budget.maxDurationMs / 60000} 分钟。</p><small>输入摘要 {analysis.inputDigest.slice(0, 16)}。仅包含展示的片段，明显凭据字段已排除；请检查其他敏感内容。</small><textarea aria-label="分析发送内容" readOnly value={analysis.input} rows={10} /><details><summary>分析指令</summary><p>{analysis.system}</p></details></details>
