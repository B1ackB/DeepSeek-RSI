...
export function observeUsage(store: Store, rsiSessions: ReadonlySet<string>, probeSessions: ReadonlySet<string> = rsiSessions, retries = new Map<string, string>(), analysisSessions: ReadonlyMap<string, string> = new Map()) {
	return async function* (options: GenerateOptions, next: () => AsyncIterable<StreamChunk>): AsyncIterable<StreamChunk> {
		...
		const analysisId = sessionId === null ? undefined : analysisSessions.get(sessionId);
		if (analysisId) {
			const analysis = store.business().analyses.find(a => a.id === analysisId) ?? store.business().mainflow.tasks.find(a => a.id === analysisId);
			...
			if (options.system !== analysis.system || options.messages.length !== 1 || options.messages[0]?.content.length !== 1 || options.messages[0].content[0]?.type !== 'text' || options.messages[0].content[0].text !== analysis.input || Buffer.byteLength(JSON.stringify([options.system, analysis.input])) > analysis.budget.maxInputBytes) throw fault('budget_config', '分析发送内容与已确认范围不符');
		}
		...
		let attempt: Attempt = {
			...
			owner: analysisId || rsi ? 'rsi' : sessionId ? 'session' : 'unresolved', ownerId: analysisId ?? (rsi ? store.snapshot().probeId : sessionId), sessionId,
			purpose: analysisId ? (store.business().mainflow.tasks.some(t => t.id === analysisId) ? 'generation' : 'analysis') : probe ? 'probe' : 'daily', callKind: options.purpose === 'compaction' ? 'compression' : 'agent', retryOfAttemptId: options.purpose === undefined && sessionId ? retries.get(sessionId) ?? null : null,
