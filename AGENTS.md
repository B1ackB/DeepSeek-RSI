...
- 当前只有规划文档；G0 的专用测试 Skill、首批调用预算与后续安装 OrbStack 已由用户确认，见 CONTRACTS.md §8。确认方案不等于已经开始实现，正式评测配置另行确定；后续执行遵守既有授权，不重复索取相同确认。
...
- 格式化、类型检查、构建和测试命令在 G0 检查现有工具后确定；不提前添加多套重叠工具链。
